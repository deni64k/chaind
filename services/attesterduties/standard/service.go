...
type Service struct {
	...
	attesterDutiesSetter chaindb.AttesterDutiesSetter
...
func New(ctx context.Context, params ...Parameter) (*Service, error) {
	...
	// Set logging.
	log = zerologger.With().Str("service", "attesterduties").Str("impl", "standard").Logger()
	if parameters.logLevel != log.GetLevel() {
		log = log.Level(parameters.logLevel)
	}

	attesterDutiesSetter, isAttesterDutiesSetter := parameters.chainDB.(chaindb.AttesterDutiesSetter)
	if !isAttesterDutiesSetter {
		return nil, errors.New("chain DB does not support attester duty setting")
	}

	s := &Service{
		eth2Client:           parameters.eth2Client,
		chainDB:              parameters.chainDB,
		attesterDutiesSetter: attesterDutiesSetter,
...
func (s *Service) catchupOnRestart(ctx context.Context, md *metadata) {
	for epoch := md.LatestEpoch; epoch <= s.chainTime.CurrentEpoch(); epoch++ {
		...
		if err := s.updateAttesterDutiesForEpoch(ctx, epoch); err != nil {
			log.Warn().Err(err).Msg("Failed to update attester duties")
...
func (s *Service) handleMissed(ctx context.Context, md *metadata) {
	...
	for i := 0; i < len(md.MissedEpochs); i++ {
		...
		if err := s.updateAttesterDutiesForEpoch(ctx, md.MissedEpochs[i]); err != nil {
			log.Warn().Err(err).Msg("Failed to update attester duties")
			failed++
			cancel()
			continue
